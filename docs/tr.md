...
3.  Система генерирует короткий код.
4.  Пользователь получает короткий код.
...
3.  **Backend Services:**
    *   **ID Generator:** "Сердце" системы, генерирующее короткие ссылки.
...
4.  Router server возвращает короткий код.
5.  **API Service** возвращает пользователю короткий код.

### Сценарий 2: Шардирование (Auto-Sharding)
