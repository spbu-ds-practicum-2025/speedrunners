...
1.  **Создание короткой ссылки (`POST /shorten`):**
    ...
    *   Возвращает уникальную короткую ссылку.
    *   В случае ошибки возвращает описание проблемы.
2.  **Редирект (`GET /{short_code}`):**
    *   Принимает короткий код.
    *   В случае успеха возвращает HTTP Redirect (301/302) на исходный ресурс.
    *   Если ссылка не найдена — возвращает страницу 404.

### Для Frontend:
3.  **Веб-интерфейс:** Простая форма ввода ссылки, кнопка "Сократить", отображение результата.
...
3.  Система обрабатывает запрос и мгновенно возвращает короткую ссылку. 

### Сценарий 2: Пользователь переходит по ссылке
1. Пользователь (или другой человек) кликает по ссылке браузере.
...
* Внутреннее состояние: Хранит в памяти зарезервированный диапазон ID (Pool), полученный от ID Generator.
...
* Принимает (ID, URL).
* Маршрутизирует запись в активный shard_N.db.
* Мониторит размер шарда и инициирует создание нового.
...
graph TD
    ...
    %% --- СЛОЙ ПРИЛОЖЕНИЯ (С БУФЕРАМИ) ---
    subgraph AppCluster ["Application Cluster (Python/FastAPI)"]
        direction TB
        
        %% ВАЖНО: Кавычки спасают от ошибки парсинга скобок []
    ...
    subgraph BackendServices ["Backend Services"]
        ...
        subgraph ID_Logic ["ID Management"]
            direction LR
            ID_Server["ID Generator Service<br/>(Batch Allocator)"]:::component
            ID_State[("Counter.wal<br/>(Persistence)")]:::state
        end

        Router_Server["Router Service<br/>(Shard Manager)"]:::component
    end
    ...
    subgraph DataLayer ["Dynamic Storage Pool (Range-Based)"]
        ...
        S0[("Shard_0.db<br/>IDs: 0 - 1M<br/>(Filled)")]:::database
        S1[("Shard_1.db<br/>IDs: 1M - 2M<br/>(Active Write)")]:::database
        SN[("Shard_N.db<br/>Pre-allocated<br/>(Waiting)")]:::newShard
    end
    ...
    %% Логика Батчей (Range Request)
    API_1 & API_2 -- "Request Range (Size=1000)" --> ID_Server
    ID_Server <--> ID_State

    %% Логика Сохранения
    API_1 & API_2 -- "Save (ID, URL)" --> Router_Server

    %% Логика Роутера
    Router_Server -- "Calc: ID // Limit" --> S0 & S1
    Router_Server -.->|Async Pre-allocation| SN

    %% Стилизация блоков
    class Clients,Ingress,AppCluster,BackendServices,DataLayer darkContainer
    class ID_Logic darkContainer
```
...
1. **API Service** получает запрос на сокращение ссылки.
2. **API Service** проверяет свой локальный буфер ID в памяти:
...
3. **API Service** передает данные в Router Service.
Далее процесс сохраняется (Router Service считает индекс шарда и пишет в БД).
...
sequenceDiagram
    ...
    Note over API: Поступил запрос POST /shorten

    alt Buffer is Empty
        API->>ID: Request Range (size=1000)
        ID->>ID: Global_Counter += 1000
        ID-->>API: Return Range [1000...1999]
        Note over API: Сохраняем [1000...1999] в RAM
    else Buffer has IDs
        Note over API: Берем следующий ID из RAM
    end

    Note over API: Current ID = 1000
    Note over API: Encoding -> "abc12"
...
*   **Задача:** Внедрить `Router Service` с логикой `ID // Limit`. API Service перестает знать о файлах БД. Он делегирует операции ввода-вывода Router Service, который реализует логику ID // Limit.
