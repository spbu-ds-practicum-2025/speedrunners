...
| **Короткий код** | Уникальная последовательность символов, идентифицирующая сокращённую ссылку (например, `a1b2c3`) |
| **Короткая ссылка** | Полный URL, состоящий из домена сервиса и короткого кода (например, `http://short.ty/a1b2c3`) |
