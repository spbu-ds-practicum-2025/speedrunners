...
Этот документ описывает сценарии проверки системы.
...
**Окружение:**
- Docker Compose Cluster (Nginx, API, Router, ID Gen).
...
1.  **Пустой ввод:** Оставить поле пустым и нажать "Сократить".
    *   *Ожидание:* Красное сообщение об ошибке.
2.  **Несуществующая ссылка:** Ввести в адресную строку `http://localhost/im1not2exist`.
    *   *Ожидание:* Браузер показывает JSON с ошибкой `{"detail": "Link not found..."}` или страницу 404.
...
**Сценарий:**
1.  Запустить скрипт, который генерирует 1200 ссылок.
...
*Проверка производительности с имитацией роста нагрузки.*
