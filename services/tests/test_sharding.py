...
async def run_test():
    ...
    for i in range(1, 1201):
        payload = {
            "url": f"http://site.com/resource_{i}"
        }
    ...
    if len(shards) >= 2:
        print("✅ PASS: Система создала несколько шардов (Динамическое шардирование работает).")
    else:
        print("⚠️ WARNING: Найден только один шард. Возможно, лимит слишком большой или база не сброшена.")
